...
class vector {
  ...
  //dangerous
  vector(const std::size_t dim, std::unique_ptr<value_type[]> elem);
...
constexpr vector::vector() : dimension{0}, arrow{nullptr} {}
      ...
      : dimension{dim}
{
  if (dim != 0) [[likely]]
  {
    arrow = std::make_unique<double[]>(dim);
    std::fill(arrow.get(), arrow.get() + dim, 0.);
  } else  throw vector_exception::non_pos();
  std::cout << "\texiting std::size_t constructor" << std::endl;
}
      ...
      : dimension{elem.size()}
{
  ...
  std::cout << "\texiting std::vector constructor" << std::endl;
      ...
      : dimension{il.size()}
{
  ...
  std::cout << "\texiting std::initializer_list constructor" << std::endl;
}
...
{
  v.dimension = 0;
  std::cout << "\texiting move constructor" << std::endl;
}
...
vector& vector::operator=(vector&& v) noexcept
{
  ...
  v.dimension = 0;
  std::cout << "\treturning from operator=" << std::endl;
  return *this;
}

[[nodiscard]] constexpr bool vector::empty() const noexcept
{
  if (this->dimension == 0)
    return true;
  else
