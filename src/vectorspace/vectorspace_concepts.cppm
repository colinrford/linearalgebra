...
namespace concepts::experimental::internals
{
  ...
  concept left_module_element_c_weak =
    lam::concepts::experimental::additive_group_element_c_weak<M>
    and lam::concepts::experimental::ring_element_c_weak<R>
    and requires(M m, R r)
    {
      { r * m } -> std::same_as<M>;
    };
   ...
   */
  template<typename M, typename R>
  concept right_module_element_c_weak =
    lam::concepts::experimental::additive_group_element_c_weak<M>
    and lam::concepts::experimental::ring_element_c_weak<R>
    and requires(M m, R r)
    {
   ...
   */
  template<typename M, typename R>
  concept module_element_c_weak =
    left_module_element_c_weak<M, R>
    and right_module_element_c_weak<M, R>;
  ...
  concept vectorspace_element_c_weak =
    module_element_c_weak<V, K>
    and lam::concepts::experimental::field_element_c_weak<K>;
  ...
  concept vector_c_weak =
    ...
    and module_element_c_weak<V, typename V::scalar_type>
    and requires(V v, V::size_type i)
    {
       { v.size() } -> std::same_as<typename V::size_type>;
       { v[i] } -> std::convertible_to<typename V::scalar_type>;
    }
    and std::same_as<
          std::remove_cvref_t<decltype(std::declval<V>()[std::declval<typename V::size_type>()])>,
          typename V::scalar_type
        >;
} // namespace internals
