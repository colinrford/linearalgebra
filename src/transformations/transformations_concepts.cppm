...
namespace lam::linalg::concepts::experimental::internals
{
  ...
  concept linear_transformation_c_weak =
    ...
    && lam::linalg::concepts::experimental::vectorspace_element_c_weak<W, typename W::scalar_type>
    && requires(F f, V v)
    {
  ...
  concept matrix_c_weak =
    ...
    && requires(M m, typename M::size_type i)
    {
      ...
      // Dimensions
      { m.get_rows() } -> std::integral;
      { m.get_cols() } -> std::integral;
  ...
  concept representation_c_weak = 
    requires(Rho rho, G g, V v)
    {
