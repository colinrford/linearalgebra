...
struct fixed_matrix
{
  ...
  constexpr explicit fixed_matrix(const std::array<T, Rows * Cols>& arr) noexcept : data{arr} {}

  // From initializer (column-major order)
  template<typename... Args>
    requires(sizeof...(Args) == Rows * Cols) && (std::convertible_to<Args, T> && ...)
  constexpr fixed_matrix(Args... args) noexcept : data{static_cast<T>(args)...}
  {}
  ...
  constexpr T& operator[](size_type row, size_type col) noexcept { return data[col * Rows + row]; }

  constexpr const T& operator[](size_type row, size_type col) const noexcept { return data[col * Rows + row]; }
  ...
  static constexpr fixed_matrix zero() noexcept { return fixed_matrix{}; }
...
constexpr fixed_matrix<T, M, P> operator*(const fixed_matrix<T, M, N>& a, const fixed_matrix<T, N, P>& b) noexcept
{
...
export template<typename T, std::size_t Rows, std::size_t Cols>
constexpr fixed_matrix<T, Rows, Cols> operator*(const T& scalar, const fixed_matrix<T, Rows, Cols>& m) noexcept
{
...
constexpr fixed_matrix<T, 4, 4> rotate(T angle, const fixed_vector<T, 3>& axis) noexcept
{
  ...
  if (len == T{0})
    return fixed_matrix<T, 4, 4>::identity();
...
export template<typename T>
constexpr fixed_matrix<T, 4, 4> look_at(const fixed_vector<T, 3>& eye, const fixed_vector<T, 3>& center,
                                        const fixed_vector<T, 3>& up) noexcept
{
  // Forward (camera looks down -Z in view space)
  fixed_vector<T, 3> f{center.x() - eye.x(), center.y() - eye.y(), center.z() - eye.z()};
  ...
  // Right = f x up
  fixed_vector<T, 3> r{f.y() * up.z() - f.z() * up.y(), f.z() * up.x() - f.x() * up.z(),
                       f.x() * up.y() - f.y() * up.x()};
  ...
  // Up = r x f
  fixed_vector<T, 3> u{r.y() * f.z() - r.z() * f.y(), r.z() * f.x() - r.x() * f.z(), r.x() * f.y() - r.y() * f.x()};
...
export template<typename T>
constexpr fixed_matrix<T, 4, 4> perspective(T fov_y_radians, T aspect, T near, T far) noexcept
{
...
export template<typename T>
constexpr fixed_matrix<T, 4, 4> ortho(T left, T right, T bottom, T top, T near, T far) noexcept
{
  fixed_matrix<T, 4, 4> m{};
...
struct std::formatter<lam::linalg::fixed_matrix<T, Rows, Cols>>
{
  ...
  auto format(const lam::linalg::fixed_matrix<T, Rows, Cols>& m, std::format_context& ctx) const
  {
    auto out = ctx.out();
    std::format_to(out, "[\n");
