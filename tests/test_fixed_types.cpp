...
constexpr bool test_fixed_vector_arithmetic()
{
  ...
  auto e = 2.0 * a;
  if (e.x() != 2.0 || e.y() != 4.0 || e.z() != 6.0)
    return false;

  auto f = a * 3.0;
  if (f.x() != 3.0 || f.y() != 6.0 || f.z() != 9.0)
    return false;

  // Scalar division
  auto g = b / 2.0;
  if (g.x() != 2.0 || g.y() != 2.5 || g.z() != 3.0)
    return false;

  // Negation
  auto h = -a;
  if (h.x() != -1.0 || h.y() != -2.0 || h.z() != -3.0)
    return false;

  // Compound assignment
  vec3d i{1.0, 1.0, 1.0};
  i += a;
  if (i.x() != 2.0 || i.y() != 3.0 || i.z() != 4.0)
    return false;
...
constexpr bool test_fixed_matrix_construction()
{
  ...
  if (m2[0, 0] != 1.0 || m2[1, 1] != 1.0 || m2[2, 2] != 1.0 || m2[3, 3] != 1.0)
    return false;
  if (m2[0, 1] != 0.0 || m2[1, 0] != 0.0)
    return false;
...
constexpr bool test_fixed_matrix_arithmetic()
{
  ...
  // Addition
  auto c = a + b;
  if (c[0, 0] != 2.0 || c[1, 1] != 2.0)
    return false;

  // Scalar multiplication
  auto d = 3.0 * a;
  if (d[0, 0] != 3.0 || d[1, 1] != 3.0)
    return false;

  // Matrix multiplication (identity * identity = identity)
  auto e = a * b;
  if (e[0, 0] != 1.0 || e[1, 1] != 1.0 || e[0, 1] != 0.0)
    return false;
...
constexpr bool test_transpose()
{
  ...
  if (t[0, 0] != 1.0 || t[0, 1] != 4.0)
    return false;
  if (t[1, 0] != 2.0 || t[1, 1] != 5.0)
    return false;
  if (t[2, 0] != 3.0 || t[2, 1] != 6.0)
    return false;
...
constexpr bool test_graphics_transforms()
{
  ...
  auto tp = t * p;
  if (tp.x() != 1.0 || tp.y() != 2.0 || tp.z() != 3.0 || tp.w() != 1.0)
    return false;
...
static_assert(lam::concepts::experimental::vector_c_weak<vec3d>, "vec3d must satisfy vector_c_weak");
static_assert(lam::concepts::experimental::vector_c_weak<vec4f>, "vec4f must satisfy vector_c_weak");
...
int main()
{
  ...
  std::println("  fixed_vector construction: {}", test_fixed_vector_construction() ? "PASS" : "FAIL");
  std::println("  fixed_vector arithmetic:   {}", test_fixed_vector_arithmetic() ? "PASS" : "FAIL");
  std::println("  fixed_vector comparison:   {}", test_fixed_vector_comparison() ? "PASS" : "FAIL");
  std::println("  fixed_matrix construction: {}", test_fixed_matrix_construction() ? "PASS" : "FAIL");
  std::println("  fixed_matrix arithmetic:   {}", test_fixed_matrix_arithmetic() ? "PASS" : "FAIL");
  std::println("  matrix-vector multiply:    {}", test_matrix_vector_multiply() ? "PASS" : "FAIL");
  std::println("  transpose:                 {}", test_transpose() ? "PASS" : "FAIL");
  std::println("  trace:                     {}", test_trace() ? "PASS" : "FAIL");
  std::println("  graphics transforms:       {}", test_graphics_transforms() ? "PASS" : "FAIL");
